...
def evaluatePrediction(filename,type_of_label,file_eval):
	...
	with open(filename, 'r') as generatedFile,\
		open('./data/'+file_eval, 'a') as evalFile:
			...
			if True: #rm['relation']['true_label'] in ['None','per:country_of_death','per:country_of_birth','per:countries_of_residence','per:nationality','per:children','org:parents','org:founded_by']:
				...
				if check_string(rm['relation'][type_of_label],rm['relation']['true_label'])==False:
					...
					if rm['relation'][type_of_label]=='None' and rm['relation']['true_label']!='None':
						...
						print 'predic_label: ',rm['relation'][type_of_label],'; true_label: ',rm['relation']['true_label'],'; label:',rm['relation']['label']
					...
					if rm['relation'][type_of_label]!='None':
						...
						true_negative+=1
		#print 'relationsTypes: ',list(set(relationsTypes))
...
if __name__ == "__main__":
	if len(sys.argv) != 3:
		print 'Usage: evaluate_prediction.py -DATA (Google/Riedel/KBP/Hoffman/)'
		exit(-1)

	_data = sys.argv[1]
	_flag = int(sys.argv[2])
	...
	# if _flag==1:
	# 	# convert_psl_prediction_to_json('/Users/zamiraniyazova/Documents/MasterStudy/Thesis/PSLDataPipeline/inferred-predicates/mlnHasrel.txt', psl_dir+'/prediction_mln.json',False)
	# 	convert_psl_prediction_to_json('/Users/zamiraniyazova/Documents/MasterStudy/Thesis/PSLDataPipeline/inferred-predicates/HASREL.txt', psl_dir+'/prediction_psl.json',True)
	# else:
	# 	convert_psl_prediction_to_json('/Users/zamiraniyazova/Documents/MasterStudy/Thesis/psl_relations/groovy/inferred-predicates/HASREL.txt', psl_dir+'/prediction_psl.json',True)
	# 	convert_psl_prediction_to_json('/Users/zamiraniyazova/Documents/MasterStudy/Thesis/psl_relations/groovy/inferred-predicates/HASREL_wl.txt', psl_dir+'/prediction_psl_wl.json',True)
		# convert_psl_prediction_to_json('/Users/zamiraniyazova/Documents/MasterStudy/Thesis/PSLDataPipeline/inferred-predicates/mlnHasrel.txt', psl_dir+'/prediction_mln.json',False)

	# print '2 step. Compare freebase to ground truth: '
	# evaluatePrediction(candidate_rm_file, 'label','evaluation_result.txt')
	# print '2 step. Compare weight learning to ground truth: '
	# evaluatePrediction(psl_dir+'/prediction_psl_wl.json','predic_label','psl_learned_rules_eval.txt')
	# print '3 step. Compare psl to ground truth: '
	# evaluatePrediction(psl_dir+'/prediction_psl.json','predic_label','psl_own_rules_eval.txt')
	print '4 step. Compare brute force to ground truth: '
	evaluatePrediction(psl_dir+'/brute_force.json','predic_label','brute_force_eval.txt')
	# print '5 step. Compare MLN to ground truth: '
	# evaluatePrediction(psl_dir+'/prediction_mln.json','predic_label','mln_rules_eval.txt')
