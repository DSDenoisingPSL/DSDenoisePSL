...
mag_vectors = Magnitude("../data/embeddings/GoogleNews-vectors-negative300.magnitude")
...
def predicateVerbSimilarity(relMention,relMention_2):
    ...
    if len(relMention['depen_struct'])>0 and len(relMention_2['depen_struct'])>0 and len(relMention['depen_struct'])<11 and len(relMention_2['depen_struct'])<11:
        ...
        if score>0.6:
            if score>1.0:
                score=1.0
            with open(outdir+'/verbsSimilarity_obser.txt', 'a', encoding="utf-8") as predFile:
                predFile.write(relMention['rm_id']+'\t'+relMention_2['rm_id']+'\t'+str(score)+'\n')
        else:
            score = 0.0

        with open(outdir+'/mln_model.db', 'a', encoding="utf-8") as mln_db:
            if score==0.0:
...
def predicateDependPathSimilarity(relMention,relMention_2):
    ...
    if len(word_set_1)>0 and len(word_set_2)>0:
        ...
        if score>0.4:
            if score>1.0:
                score=1.0
            with open(outdir+'/depenPathSimilarity_obser.txt', 'a', encoding="utf-8") as predFile:
                predFile.write(relMention['rm_id']+'\t'+relMention_2['rm_id']+'\t'+str(score)+'\n')
        else:
            score = 0.0
...
def predicateDependPathStructureSimilarity(relMention,relMention_2):
    ...
    if score>0.6:
        if score>1.0:
            score=1.0
        with open(outdir+'/depenStructureSimilarity_obser.txt', 'a', encoding="utf-8") as predFile:
            predFile.write(relMention['rm_id']+'\t'+relMention_2['rm_id']+'\t'+str(score)+'\n')
    else:
        score = 0.0
    ...
    return score
# ===============
## create file relation_mentions similar to original (cotype) but with relationf from freebase
...
def pslPredicateGeneration(outdir,jsonFname,featuresFile):
    ...
    relation_arr = []
    index = 0
    with open(outdir+'/verbsSimilarity_obser.txt', 'w', encoding="utf-8") as sim1,\
    open(outdir+'/depenPathSimilarity_obser.txt', 'w', encoding="utf-8") as sim2,\
    open(outdir+'/depenStructureSimilarity_obser.txt', 'w', encoding="utf-8") as sim3:
        sim1.close()
        sim2.close()
        sim3.close()

    with open(outdir+'/commonWords_obser.txt', 'w', encoding="utf-8") as commonWordsFile,\
    open(outdir+'/HasDirectDependency_obser.txt', 'w', encoding="utf-8") as hasDirectDependencyFile,\
    open(outdir+'/HasSimilarEntityVerbs_obser.txt', 'w', encoding="utf-8") as hasSimilarEntityVerbsFile,\
    open(outdir+'/ShortDependPath_obser.txt', 'w', encoding="utf-8") as shortDependPathFile,\
    open(outdir+'/HasRel_true.txt', 'w', encoding="utf-8") as trueFile,\
    open(outdir+'/CandRel_obser.txt', 'w', encoding="utf-8") as candRelFile,\
    open(outdir+'/CandRel_target.txt', 'w', encoding="utf-8") as targetFile,\
    open(outdir+'/Ent1SpaCyType.txt', 'w', encoding="utf-8") as ent1type, \
    open(outdir+'/Ent2SpaCyType.txt', 'w', encoding="utf-8") as ent2type,\
    open(outdir+'/Ent1StanType.txt', 'w', encoding="utf-8") as ent1type_st, \
    open(outdir+'/Ent2StanType.txt', 'w', encoding="utf-8") as ent2type_st,\
    open(outdir+'/isTitleEnt1.txt', 'w', encoding="utf-8") as ent1cap, \
    open(outdir+'/isTitleEnt2.txt', 'w', encoding="utf-8") as ent2cap,\
    open(outdir+'/mln_model.db', 'w', encoding="utf-8") as mln_db,\
    open(featuresFile, 'r') as fin:
        ...
        for line in fin:
            ...
            commonWordsFile.write(f"{relMention['rm_id']}'\t'{str(relMention['common_words'])}'\n'")
            hasDirectDependencyFile.write(f"{relMention['rm_id']}'\t'{str(relMention['nouns_type'])}'\n'")
            hasSimilarEntityVerbsFile.write(f"{relMention['rm_id']}'\t'{str(relMention['ent_verbs_sim'])}'\n'")


            mln_db.write(MLNPredicates(relMention['nouns_type'],'HasDirectDependency(p'+str(relMention['rm_id'])+')\n'))
            mln_db.write(MLNPredicates(relMention['ent_verbs_sim'],'HasSimilarEntityVerbs(p'+str(relMention['rm_id'])+')\n'))

            if relMention['path_length']>8:
                mln_db.write('!DependencyPathLength(p'+str(relMention['rm_id'])+')\n')
            else:
            ...
            if len(relMention['short_dep_path'])>0 and len(relMention['depen_struct'])>0:
                ...
                if sentence!='':
                    shortDependPathFile.write(str(relMention['rm_id'])+'\t'+str(relMention['short_dep_path'])+'\n')


                    if not check_string_over('died,dead,dies,death,die', relMention['short_dep_path']):
                        mln_db.write('!HasWordsShortDependPath(p'+str(relMention['rm_id'])+',death_words)\n')
                    if not check_string_over('born,birth,native', relMention['short_dep_path']):
                        mln_db.write('!HasWordsShortDependPath(p'+str(relMention['rm_id'])+',birth_words)\n')
                    if check_string_over('rear,raise,lived,live,moved,move,work,home,lives', relMention['short_dep_path']):
                        mln_db.write('!HasWordsShortDependPath(p'+str(relMention['rm_id'])+',lived_words)\n')
                    if not check_string_over('son,daughter,child,mother,father,parent', relMention['short_dep_path']):
                        mln_db.write('!HasWordsShortDependPath(p'+str(relMention['rm_id'])+',children_words)\n')
                    if not check_string_over('found,founded,founde', relMention['short_dep_path']):
                        mln_db.write('!HasWordsShortDependPath(p'+str(relMention['rm_id'])+',founders_words)\n')
                    if not check_string('enrol,study,complete,graduate,attend,studied,complete,alumni,university,institute,graduate,school', relMention['short_dep_path']):
                        mln_db.write('!HasWordsShortDependPath(p'+str(relMention['rm_id'])+',institution_words)\n')
                    if not check_string('bachelor,master,doctor,received', relMention['short_dep_path']):
                        mln_db.write('!HasWordsShortDependPath(p'+str(relMention['rm_id'])+',degree_words)\n')
                    ...
                    if check_string_over('of', relMention['short_dep_path']):
                        mln_db.write('HasWordsShortDependPath(p'+str(relMention['rm_id'])+',lived_words_of)\n')
                    ...
                    if check_string_over('ceo,director,chief,worker,member,corp,president,chairman', relMention['short_dep_path']):
                        mln_db.write('HasWordsShortDependPath(p'+str(relMention['rm_id'])+',company_words)\n')
            ...
            for label in labels:
                candRelFile.write(str(relMention['rm_id'])+'\t'+label+'\n')
                mln_db.write('DSCandRel(p'+str(relMention['rm_id'])+','+label.replace(":", "1")+')\n')
            
            for relt in relationTypes:
                relt=relt.strip('\r\n')
                targetFile.write(str(relMention['rm_id'])+'\t'+relt+'\n')
                if relt not in labels:
                    mln_db.write('!DSCandRel(p'+str(relMention['rm_id'])+','+relt.replace(":", "1")+')\n')


            labels = relMention['relation']['true_label'].split(",")
            trueFile.write(str(relMention['rm_id'])+'\t'+labels[0]+'\n')
            ...
            for item in relMention['entityTypes']:
                if relMention['relation']['em1Text'] in item:
                    if (relMention['rm_id'],item[relMention['relation']['em1Text']]['label']) not in ent1TypeSet:
                        ...
                        ent1type.write(str(relMention['rm_id'])+'\t'+item[relMention['relation']['em1Text']]['label']+'\n')
                        ent1type_st.write(str(relMention['rm_id'])+'\t'+item[relMention['relation']['em1Text']]['st_label']+'\n')
                        mln_db.write('Ent1TypeSp(p'+str(relMention['rm_id'])+','+item[relMention['relation']['em1Text']]['label']+')\n')
                        mln_db.write('Ent1TypeSt(p'+str(relMention['rm_id'])+','+item[relMention['relation']['em1Text']]['st_label']+')\n')
                        if(any(x.isupper() for x in relMention['relation']['em1Text'])):
                            val = 1
                            mln_db.write('Ent1Capital(p'+str(relMention['rm_id'])+')\n')
                        else:
                            val = 0
                            mln_db.write('!Ent1Capital(p'+str(relMention['rm_id'])+')\n')
                        ent1cap.write(str(relMention['rm_id'])+'\t'+str(val)+'\n')
                ...
                if relMention['relation']['em2Text'] in item:
                    if (relMention['rm_id'],item[relMention['relation']['em2Text']]['label']) not in ent2TypeSet:
                        ...
                        ent2type.write(str(relMention['rm_id'])+'\t'+item[relMention['relation']['em2Text']]['label']+'\n')
                        ent2type_st.write(str(relMention['rm_id'])+'\t'+item[relMention['relation']['em2Text']]['st_label']+'\n')
                        mln_db.write('Ent2TypeSp(p'+str(relMention['rm_id'])+','+item[relMention['relation']['em2Text']]['label']+')\n')
                        mln_db.write('Ent2TypeSt(p'+str(relMention['rm_id'])+','+item[relMention['relation']['em2Text']]['st_label']+')\n')
                        if(any(x.isupper() for x in relMention['relation']['em2Text'])):
                            val = 1
                            mln_db.write('Ent2Capital(p'+str(relMention['rm_id'])+')\n')
                        else:
                            val = 0
                            mln_db.write('!Ent2Capital(p'+str(relMention['rm_id'])+')\n')
                        ent2cap.write(str(relMention['rm_id'])+'\t'+str(val)+'\n')
        ...
        if _dir=='test':
            with open(outdir+'/similarity_target.txt', 'w', encoding="utf-8") as predFile:
        ...
        with open(outdir+'/ent1_target.txt', 'w', encoding="utf-8") as ent1_target,\
        open(outdir+'/ent2_target.txt', 'w', encoding="utf-8") as ent2_target:
            for i, rm1 in enumerate(relation_arr):
...
#dependency path length
        ...
        with open(outdir+'/dependPathLength_obser.txt', 'w', encoding="utf-8") as predFile:
...
def candidateRelMentionsGeneration(outdir,jsonFname,cand_file):
    ...
    with open(jsonFname, 'r') as fin, open(outdir+'/relationTypes.txt', 'w') as relType,open(cand_file, 'w', encoding="utf-8") as outfile:
        rmDependPath = {}
        relationsTypes = []
        start = time.time()
        indexId=0
        for line in fin:
            ...
            for i, rm in enumerate(sentDic['relationMentions']):
                ...
                sys.stdout.write("Processed %d  relation-mention Time: %d sec\r" % (indexId, time.time() - start))
                sys.stdout.flush() 
                ...
                for label in labels:
                    relationsTypes.append(label.strip('\r\n'))
        outfile.close()
        print(f"read data from {jsonFname}  sample entry: \n{line.encode('utf-8')}")
        print(f"wrote candidate-relations to file '{cand_file}'\n ##########\n sample entry: \n{str(sentDic).encode('utf-8')}")
        #write all relations type to file       
        rm = list(set(relationsTypes))
        print(f"wrote relation-types to file '{outdir}/relationTypes.txt'")
        relType.write("\n".join(rm))
        relType.close()
        print ('\n')
...
if __name__ == "__main__":
    ...
    start=time.time()
    # read file from /test.json
    #write open(outdir + '/relationTypes.txt', 'w') as relType, open(cand_file, 'w') as outfile
